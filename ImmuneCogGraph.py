...
class ImmuneCogGraph(CogGraph):
    ...
    def __init__(self, rl_agent, device="cpu", env=None):
        ...
        self.visit_age_map = torch.zeros_like(self.env.infected_map, dtype=torch.int16)
        ...
        for u in self.units:
            if u.role == "emitter":
                if not hasattr(u, "position"):
                    u.position = random.choice(self.free_positions)
                u.output_positions = deque(maxlen=20)   # 轨迹用于惩罚
        # --------------------------------------------------------------
        # --- 元学习 trainer（假设你已经按前面示例实现过 MetaTrainer）---

        self.meta_trainer = MetaTrainer(self.immune_processor)
    ...
    def _update_target_vector(self):
        ...
        age_map = self.visit_age_map.float()  # (H,W)
        ...
        if hasattr(self.env, "infected_map"):
            ...
            local_inf = torch.zeros_like(full_inf)
            for u in self.units:
                if u.role != "emitter" or not hasattr(u, "position"):
                    continue
                x, y = u.position
                xs = slice(max(0, x-4), min(self.env.size, x+5))
                ys = slice(max(0, y-4), min(self.env.size, y+5))
                local_inf[ys, xs] = full_inf[ys, xs]
        ...
        if hasattr(self.env, "privilege_level"):
            privileged = (self.env.privilege_level > 0.5).float().view(1, -1)
        else:
    ...
    def _rebuild_free_positions(self):
        """
        重构可出生位置：未感染 & 未隔离 & 当前没有单位
        """
        free_positions = []
        for x in range(self.env.size):
            for y in range(self.env.size):
                if self.env.infected_map[y, x] > 0.0: continue
                if self.env.is_quarantined[y, x] > 0.0: continue
                pos_taken = any(u.get_position() == (x, y) for u in self.units)
                if pos_taken: continue
    ...
    def sensor_forward(self, flat_state: torch.Tensor):
        ...
        out = self.sensor_net(flat_state)          # [B, hidden_s]
    ...
    def _assign_emitter_goal(self, u):
        ...
        # --- 1) 本地可见目标 ---
        if hacked.sum() > 0:
            pos_list = torch.nonzero(hacked > 0.5)
            goal_type = "hack"
        elif infected.sum() > 0:
        ...
        # ---------------------- 3) 正常选目标 --------------------
        y, x = random.choice(pos_list.tolist())
        u.personal_goal = (x, y)
    ...
    def processor_forward(self, sensor_out: torch.Tensor):
        ...
        proc_out = self.processor_net(sensor_out)
    ...
    def step(self, input_tensor: torch.Tensor):
        ...
        # —— 缓存一次，全局共享 —— #
        tv_cached = self.target_vector.detach()
        self.tv_cached = tv_cached
        for u in self.units:
            if u.role == "emitter":
                u.goal_vec = tv_cached
        ...
        full_state = torch.cat(
            [env_state, unvisited_map, infected_map, privileged_map],
            dim=1
        )
        ...
        if self.current_step % 500 == 0 and len(self.replay_buffer) >= 100:
            ...
            # states: List[Tensor(d_model,)] -> (B, d_model)
    ...
    def report_antibody_stats(self):
        ...
        if self.current_step % 100 == 0:
            logger.warning(
                f"[抗体统计] 成功 {self.antibody_success_count} 次，失败 {self.antibody_failure_count} 次，成功率 {success_rate:.2%}")
    ...
    def _apply_defense_rewards(self,
        ...
        priv_positions = {
            (x.item(), y.item())
            for y, x in torch.nonzero(self.env.privilege_level > 0.5)
        }
        penalty_per_node = 0.07  # 每个未清除提权节点，对应 emitter 扣的能量
        ...
        # —— 6) 黑客防御奖励 —— #
        cleared_priv = (prev_priv > 0.5).sum() - (self.env.privilege_level > 0.5).sum()
    ...
    def _run_emitter_actions(self):
        for unit in self.units:
            ...
            # ---------- ★PATCH 2: 先移动，再立即重新解码一次 ----------
            if action and action["type"] == "move":
                unit.position = action["target"]
                # 重新根据 **新位置** 解码一次，看看能不能直接清理
                action = self._decode_action_from_output(unit, action_vec)
            # -----------------------------------------------------------

            if not action or action["type"] == "move":
                continue  # 仍然只是移动，则本回合结束

            infected_before = self.env.infected_map[action["target"][1],
            action["target"][0]].item()
            self.emitter_actions.perform(action)
            infected_after = self.env.infected_map[action["target"][1],
            action["target"][0]].item()
            if infected_before > 0.5 and infected_after == 0.0:
                unit.cleared_positions.add(tuple(action["target"]))
                logger.warning(f"[清除记录] emitter {unit.id} 在 {action['target']} 清除了一个病毒")
    ...
    def _decode_action_from_output(self, unit, output_vec):
        ...
        if self.env.privilege_level[y, x] > 0.5:
