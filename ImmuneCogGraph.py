...
class ImmuneCogGraph(CogGraph):
    ...
    def _resize_full_state_dependents(
        ...
        state_channels = state_channels if state_channels is not None else self._STATE_CHANNELS
        hack_channels = hack_channels if hack_channels is not None else self._HACK_CHANNELS
        ...
        if getattr(self, "_full_state_buf", None) is None or self._full_state_buf.shape[1] != total_dim:
        ...
        if getattr(self, "_hack_onehot", None) is None or self._hack_onehot.shape[1] != D_hack:
            old_hack = getattr(self, "_hack_onehot", None)
            new_hack = torch.zeros(1, D_hack, device=self.device)
        ...
        self._HACK_CHANNELS = hack_channels
        self._INPUT_CHANNELS = total_channels
        self._D_env = D_env
        self._D_hack = D_hack
        self._D_goal = D_goal
        self._S2 = seq_len
