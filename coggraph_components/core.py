...
from env import GridEnvironment, logger
from goal_generator import make_onehot, sample_unvisited
from self_model import build_self_model

from CogUnit import CogUnit
from coggraph_components.constants import (
    DENOM,
    HIT_THRESH,
    IDEAL_RATIO,
    INPUT_CHANNELS,
...
from coggraph_components.transformer import build_transformer_block
from coggraph_components.utils import align_vec
...
class CogGraph:
    ...
    def _init_shared_tx(self):
        ...
        blocks = [build_transformer_block(D, H, self.device) for _ in range(L)]
    ...
    def merge_redundant_units(self, max_merge_cells=100):
        ...
        for u1 in limited_units:
            ...
            for u2 in neighbors:
                ...
                # ---- 1) 对齐 state ----
                s1, s2, L = align_vec(u1.state.view(-1), u2.state.view(-1), mode="pad")
                ...
                o1, o2, _ = align_vec(u1.last_output.view(-1), u2.last_output.view(-1), mode="pad")
